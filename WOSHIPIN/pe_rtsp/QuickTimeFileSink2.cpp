...
#define STANDARD_QTFILESINK 0	//xinghua 20100417
//IMPORTANT NOTES: It's better compile this file with same compiler options as livemedia.
//e.g. UNICODE is not enabled by default for livemedia build
...
public:
  SubsessionIOState(QuickTimeFileSink2& sink, MediaSubsession& subsession);
...
public:
  ...
  typedef unsigned (QuickTimeFileSink2::*atomCreationFunc)();
...
////////// QuickTimeFileSink2 implementation //////////

QuickTimeFileSink2::QuickTimeFileSink2(UsageEnvironment& env,
...
#if STANDARD_QTFILESINK //xinghua
...
QuickTimeFileSink2::~QuickTimeFileSink2() {
  completeOutputFile();
...
QuickTimeFileSink2*
QuickTimeFileSink2::createNew(UsageEnvironment& env,
			     MediaSession& inputSession,
			     char const* outputFileName,
			     unsigned bufferSize,
			     unsigned short movieWidth,
			     ...
			     Boolean generateHintTracks,
			     Boolean generateMP4Format) {
  QuickTimeFileSink2* newSink = 
    new QuickTimeFileSink2(env, inputSession, outputFileName, bufferSize, movieWidth, movieHeight, movieFPS,
...
//xinghua added 20100416
void QuickTimeFileSink2::WritePacket(void *pkt)
{
	RTP_PLAYLOAD_PACKET *rtppkt = (RTP_PLAYLOAD_PACKET*)pkt;
  ...
  while ((subsession = iter.next()) != NULL) {
				...
				strcmp(subsession->codecName(), "H264") == 0 ) 
	{
		...
		if(toSize >  rtppkt->len)
			toSize = rtppkt->len;

		memcpy(toPtr, rtppkt->buf, toSize);

		ioState->afterGettingFrame(toSize, rtppkt->timestamp);
	}
...
//xinghua:Don't use
Boolean QuickTimeFileSink2::startPlaying(afterPlayingFunc* afterFunc,
...
//xinghua:Don't use
Boolean QuickTimeFileSink2::continuePlaying() {
...
//xinghua:Don't use
void QuickTimeFileSink2::onSourceClosure(void* clientData) {
  SubsessionIOState* ioState = (SubsessionIOState*)clientData;
  ioState->onSourceClosure();
}

//xinghua:Don't use
void QuickTimeFileSink2::onSourceClosure1() {
...
void QuickTimeFileSink2::onRTCPBye(void* clientData) {
  SubsessionIOState* ioState = (SubsessionIOState*)clientData;
...
void QuickTimeFileSink2::completeOutputFile() {
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink2& sink,
				     MediaSubsession& subsession)
...
Boolean SubsessionIOState::setQTstate() {
  ...
  do {
    ...
    if (isHintTrack()) {
      ...
      fQTMediaInformationAtomCreator = &QuickTimeFileSink2::addAtom_gmhd;
      fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_rtp;
      ...
      fQTMediaInformationAtomCreator = &QuickTimeFileSink2::addAtom_smhd;
      fQTMediaDataAtomCreator
	= &QuickTimeFileSink2::addAtom_soundMediaGeneral; // by default
	...
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_genericMedia;
	...
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_Qclp;
	fQTSamplesPerFrame = 160;
      } else if (strcmp(fOurSubsession.codecName(), "MPEG4-GENERIC") == 0 ||
		 strcmp(fOurSubsession.codecName(), "MP4A-LATM") == 0) {
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_mp4a;
	...
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_dummy;
	fQTEnableTrack = False; // disable this track in the movie
      ...
      fQTMediaInformationAtomCreator = &QuickTimeFileSink2::addAtom_vmhd;
	...
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_h263;
	fQTTimeScale = 600;
	fQTTimeUnitsPerSample = fQTTimeScale/fOurSink.fMovieFPS;
      } else if (strcmp(fOurSubsession.codecName(), "H264") == 0) {
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_avc1;
	fQTTimeScale = 600;
	fQTTimeUnitsPerSample = fQTTimeScale/fOurSink.fMovieFPS;
      } else if (strcmp(fOurSubsession.codecName(), "MP4V-ES") == 0) {
	fQTMediaDataAtomCreator = &QuickTimeFileSink2::addAtom_mp4v;
	fQTTimeScale = 600;
	fQTTimeUnitsPerSample = fQTTimeScale/fOurSink.fMovieFPS;
      } else {
...
void SubsessionIOState::afterGettingFrame(unsigned packetDataSize,
  ...
  if (fQTMediaDataAtomCreator == &QuickTimeFileSink2::addAtom_genericMedia){
  ...
  } else  if (fQTMediaDataAtomCreator == &QuickTimeFileSink2::addAtom_Qclp) {
...
void SubsessionIOState::useFrame(SubsessionBuffer& buffer) {
  ...
  Boolean avcHack = fQTMediaDataAtomCreator == &QuickTimeFileSink2::addAtom_avc1;
...
Boolean SubsessionIOState::syncOK(struct timeval presentationTime) {
  QuickTimeFileSink2& s = fOurSink; // abbreviation
  ...
  if (s.fNumSyncedSubsessions < s.fNumSubsessions) {
	...
	if (fQTMediaDataAtomCreator == &QuickTimeFileSink2::addAtom_avc1) {
...
unsigned QuickTimeFileSink2::addWord64(u_int64_t word) {
...
unsigned QuickTimeFileSink2::addWord(unsigned word) {
...
unsigned QuickTimeFileSink2::addHalfWord(unsigned short halfWord) {
  addByte((unsigned char)(halfWord>>8)); addByte((unsigned char)halfWord);

  return 2;
}

unsigned QuickTimeFileSink2::addZeroWords(unsigned numWords) {
...
unsigned QuickTimeFileSink2::add4ByteString(char const* str) {
  addByte(str[0]); addByte(str[1]); addByte(str[2]); addByte(str[3]);

  return 4;
}

unsigned QuickTimeFileSink2::addArbitraryString(char const* str,
  ...
  if (oneByteLength) {
    ...
    if (strLength >= 256) {
      envir() << "QuickTimeFileSink2::addArbitraryString(\""
...
unsigned QuickTimeFileSink2::addAtomHeader(char const* atomName) {
...
unsigned QuickTimeFileSink2::addAtomHeader64(char const* atomName) {
...
void QuickTimeFileSink2::setWord(int64_t filePosn, unsigned size) {
  do {
  ...
  envir() << "QuickTimeFileSink2::setWord(): fseek failed (err "
	  << envir().getErrno() << ")\n";
}

void QuickTimeFileSink2::setWord64(int64_t filePosn, u_int64_t size) {
  do {
    if (SeekFile64(fOutFid, filePosn, SEEK_SET) < 0) break;
    addWord64(size);
    if (SeekFile64(fOutFid, 0, SEEK_END) < 0) break; // go back to where we were
...
#define addAtom(name) \
    unsigned QuickTimeFileSink2::addAtom_##name() { \
...
unsigned QuickTimeFileSink2::addAtom_hdlr2() {
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_genericMedia() {
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_soundMediaGeneral() {
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_Qclp() {
...
unsigned QuickTimeFileSink2::addAtom_mp4a() {
	unsigned size = 0;
  // The beginning of this atom looks just like a general Sound Media atom,
  // except with a version field of 1:
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_rtp() {
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_sdp() {
  int64_t initFilePosn = TellFile64(fOutFid);
...
unsigned QuickTimeFileSink2::addAtom_dummy() {
    int64_t initFilePosn = TellFile64(fOutFid);
